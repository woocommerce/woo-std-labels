...
GitHub action to sync a repo's issue labels with the standard WooCommerce set.
...
:warning: Whenever an issue is opened, the workflow will execute. This overrides all labels in the repository and may result in loss of data if it's run on a repo that isn't owned by WooCommerce.

You can do this by running the following command from a repo:

```bash
mkdir -p .github/workflows && curl https://raw.githubusercontent.com/woocommerce/woo-std-labels/v1/example.yml --output .github/workflows/sync-repo-labels.yml
```
...
Work should be based on the `trunk` branch, with changes PRed in.
...
If your changes ARE breaking, then you should create a `v2` branch based on trunk and update your chosen repo to use the new workflow.
